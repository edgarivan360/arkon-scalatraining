...
import graphql.{GraphQL, GraphQLRoutes, SangriaGraphQL}
import org.http4s._
...
import schema.{DeferredResolverINEGI, MutationType, QueryType}
...
object Main extends IOApp {
  ...
  def transactor[F[_]: Async: ContextShift](blocker: Blocker): Resource[F, HikariTransactor[F]] =
  ...
  def graphQL[F[_]: Effect: ContextShift: Logger](transactor: Transactor[F], blockingContext: ExecutionContext): GraphQL[F] =
    SangriaGraphQL[F](
      Schema(
        query    = QueryType[F],
        mutation = Some(MutationType[F])
      ),
      DeferredResolverINEGI[F],
